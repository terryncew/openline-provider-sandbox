...
A real disposable GitHub merge is acknowledged exactly once. Immediate
settlement is deliberately interrupted after that acknowledgement is durably
journaled. A fresh receiver must recover the exact acknowledged effect by
reading the acknowledged commit directly, issue signed effect + closure
evidence, and never replay the mutation.

Unlike LIVE-002, PASS does not depend on GitHub's PR ``merge_commit_sha`` field
having converged. A non-null contradictory PR SHA still fails closed.
"""
...
EXPERIMENT = "GITHUB-ACK-CLOSURE-LIVE-003"
PASS = "GITHUB_ACK_DIRECT_COMMIT_CLOSURE_ENFORCED"
FAIL = "GITHUB_ACK_DIRECT_COMMIT_CLOSURE_FAILED"
INCONCLUSIVE = "GITHUB_ACK_DIRECT_COMMIT_CLOSURE_UNRESOLVED"
WALLET_PIN = "4d26040c0f8e835184ebb710356c9601842610fe"
SANDBOX = "terryncew/openline-provider-sandbox"
CONFIRM = "RUN GITHUB ACK DIRECT CLOSURE"
SUBJECT = "worker-ack-closure-003"
MANDATE = "github-ack-closure-live-003"
SHA = re.compile(r"^[0-9a-f]{40}$")
RUN_ID = re.compile(r"^[1-9][0-9]{0,19}$")

ROOT = Path(__file__).resolve().parents[1]
PROVIDER_LIVE = ROOT / "scripts" / "provider_live.py"
PREREG = ROOT / "PREREGISTRATION_GITHUB_ACK_CLOSURE_LIVE_003.json"
...
def load_provider_live():
    spec = importlib.util.spec_from_file_location(
        "provider_effect_live_bootstrap", PROVIDER_LIVE
    )
...
class CountingClient:
    """Count merge mutations; all other methods are provider reads."""
...
def bootstrap(api: Any, run_id: str, provider_live: Any) -> dict[str, Any]:
    ...
    require(
        isinstance(base_sha, str) and SHA.fullmatch(base_sha) is not None,
        "BASE_SHA_INVALID",
    )
    ...
    commit = api.put_test_file(
        head,
        run_id,
        "OpenLine GitHub direct-commit closure test " + run_id + "\n",
    )
    head_sha = commit["commit"]["sha"]
    require(
        isinstance(head_sha, str) and SHA.fullmatch(head_sha) is not None,
        "TEST_COMMIT_INVALID",
    )
    ...
    pr = api.request(
        ...
        "/pulls",
        {
            "title": "Disposable GitHub direct-commit closure test " + run_id,
            "head": head,
            "base": base,
            "body": "Harmless disposable PR for GITHUB-ACK-CLOSURE-LIVE-003.",
            "draft": False,
...
def _new_gate(
    ...
    gate = EffectGate("github-ack-closure-live-003", gate_key=gate_key)
    gate.pin_principal(wallet.principal_id, wallet.root_public_key)
    gate.admit_bundle(bundle)
    return gate


def _direct_commit_observation(client: Any, target: MergeTarget, sha: str) -> dict[str, Any]:
    value = client.commit(target, sha)
    try:
        parents = [row["sha"] for row in value["parents"]]
        commit_sha = value["sha"]
    except (KeyError, TypeError) as exc:
        raise ExperimentError("DIRECT_COMMIT_RESPONSE_INVALID") from exc
    require(
        commit_sha == sha
        and len(parents) == 2
        and parents[0] == target.base_sha
        and parents[1] == target.head_sha,
        "DIRECT_COMMIT_BINDING_INVALID",
    )
    return {"sha": commit_sha, "parents": parents}
...
def run_boundary(
    ...
    *,
    ...
    require(
        state != output
        and state not in output.parents
        and output not in state.parents,
    ...
    evidence: dict[str, Any] = {
        "schema": "openline.github-ack-closure-live-003.evidence.v1",
    ...
    prepared = receiver.prepare(
        _presentation(receiver, wallet, holder, bundle)
    )
    require(prepared.get("decision") == "PREPARED", "PREPARE_FAILED")

    # Force loss of only the immediate settlement read path. GitHub's merge
    # response is recorded first by GitHubMergeReceiver.finish.
    forced_error = None
    with patch(
    ...
    require(
        forced_error == "GITHUB_MERGE_NOT_RECONCILED",
        "FORCED_INTERRUPTION_NOT_OBSERVED",
    )
    ...
    recovered = GitHubMergeReceiver(
        recovered_gate, counted, target, journal
    )

    late = settle_acknowledged_merges(
        recovered, wait_seconds=60.0
    )
    write_json(output / "late-settlement.json", late)

    confirmed = [
        row for row in late if row.get("status") == "CONFIRMED"
    ]
    if len(confirmed) != 1:
        evidence["recovery"] = {
    ...
    frontier_valid, frontier_reason = verify_record(
        result["receipt"],
        expected_public_key=effect["gate_public_key"],
    )
    ...
    # These are fresh reads only. PR merge_commit_sha is recorded for
    # diagnostics, but PASS does not wait on it. If it is non-null and
    # contradictory, the receiver's repaired reconciliation already fails.
    after_pr = _snapshot(counted.pr(target), target)
    direct_commit = _direct_commit_observation(
        counted, target, ack_sha
    )
    write_json(output / "provider-pr-after-recovery.json", after_pr)
    write_json(
        output / "provider-commit-after-recovery.json",
        direct_commit,
    )
    ...
    evidence["recovery"] = {
        ...
        "confirmed_effect_hashes": closure.get(
            "confirmed_effect_hashes"
        ),
        "unattributed_merge_observations": closure.get(
            "unattributed_merge_observations"
        ),
        "provider_pr_after": after_pr,
        "provider_direct_commit": direct_commit,
    }

    pr_identity_ok = (
        after_pr.get("head_sha") == target.head_sha
        and after_pr.get("base_ref") == target.base_ref
        and after_pr.get("repository_id") == target.repository_id
        and after_pr.get("merge_commit_sha") in {None, ack_sha}
    )
    ...
    known = (
        ...
        and effect["merge_commit"].get("sha") == ack_sha
        and effect["merge_commit"].get("parents", [None, None])[0]
        == target.base_sha
        and effect["merge_commit"].get("parents", [None, None])[1]
        == target.head_sha
        and effect["merge_commit"].get("base_drift") is False
        and direct_commit["sha"] == ack_sha
        and direct_commit["parents"]
...
def add_independent_provider_observation(
    ...
    effect = json.loads(
        (output / "effect-receipt.json").read_text(encoding="utf-8")
    )
    current = api.pr(plan["number"])
    commit = api.commit(effect["merge_commit_sha"])
    parents = [row["sha"] for row in commit["parents"]]
    observed_pr_sha = current.get("merge_commit_sha")
    observation = {
        "pr_number": plan["number"],
        "pr_state": current.get("state"),
        "pr_merged": current.get("merged"),
        "pr_merge_commit_sha": observed_pr_sha,
        "head_sha": current.get("head", {}).get("sha"),
        "base_ref": current.get("base", {}).get("ref"),
        "repository_id": current.get("base", {})
        .get("repo", {})
        .get("id"),
    ...
    write_json(
        output / "independent-provider-read.json", observation
    )

    exact = (
        observation["head_sha"] == plan["head_sha"]
        ...
        and parents == [plan["base_sha"], plan["head_sha"]]
        and observed_pr_sha in {
            None,
            effect["merge_commit_sha"],
        }
    )
...
def write_result(
    ...
    *,
    ...
    names = [
        ...
        if p.is_file()
        and p.name not in {"result.json", "SHA256SUMS.txt"}
    ]
    report = {
        "schema": "openline.github-ack-closure-live-003.result.v1",
        "experiment_id": EXPERIMENT,
        ...
        "files_sha256": {
            name: sha256(output / name) for name in names
        },
        "production_effect": False,
        "real_money": False,
        "mutation_retry_allowed": False,
        "claim_boundary": (
            "One real disposable GitHub PR merge; deliberate receiver "
            "settlement interruption after concrete acknowledged SHA; "
            "fresh receiver restart; direct exact-commit read-only "
            "attribution; signed effect and closure evidence."
        ),
    }
...
def verify(output: Path, *, require_live: bool = False) -> None:
    ...
    require(
        report["schema"]
        == "openline.github-ack-closure-live-003.result.v1",
        "RESULT_SCHEMA_INVALID",
    )
    require(
        report["experiment_id"] == EXPERIMENT
        and evidence["experiment_id"] == EXPERIMENT,
        "EXPERIMENT_ID_MISMATCH",
    )
    require(
        report["verdict"] == evidence["verdict"],
        "VERDICT_MISMATCH",
    )
    require(
    ...
    require(
        report["wallet_commit"] == WALLET_PIN,
        "WALLET_PIN_MISMATCH",
    )
    require(
        report["mutation_retry_allowed"] is False,
        "MUTATION_RETRY_POLICY_INVALID",
    )
    if require_live:
        require(report["mode"] == "live", "LIVE_RESULT_REQUIRED")
    require(
        report["preregistration_sha256"] == sha256(PREREG),
    ...
    for name, expected in report["files_sha256"].items():
        require(
            (output / name).is_file()
            and sha256(output / name) == expected,
            "EVIDENCE_HASH_MISMATCH",
        )

    require(
        evidence["interruption"]["merge_requests"] == 1,
        "MERGE_REQUEST_CARDINALITY",
    )
    require(
        evidence["interruption"]["provider_acknowledged"] is True,
    ...
    require(
        evidence["interruption"]["journal_status"] == "UNCERTAIN",
        "INTERRUPTION_STATE_INVALID",
    )
    require(
        evidence["interruption"]["effect_receipt_present"] is False,
        "EFFECT_PREMATURELY_PRESENT",
    )
    ...
    if report["verdict"] == PASS:
        ...
        closure = json.loads(
            (output / "closure-receipt.json").read_text(encoding="utf-8")
        )
        frontier = json.loads(
            (output / "frontier-receipt.json").read_text(encoding="utf-8")
        )
        admission = json.loads(
            (output / "admission-receipt.json").read_text(encoding="utf-8")
        )
        key = effect["gate_public_key"]
        for value in (effect, closure, frontier, admission):
            valid, _reason = verify_record(
                value, expected_public_key=key
            )
            require(valid is True, "SIGNED_EVIDENCE_INVALID")

        require(
            evidence["recovery"]["receiver_restarted"] is True,
            "RESTART_NOT_PROVED",
        )
        require(
            evidence["recovery"]["merge_requests"] == 1,
            "MERGE_RETRIED",
        )
        require(
            effect["status"] == "MERGE_CONFIRMED",
            "EFFECT_STATUS_INVALID",
        )
        require(
            effect["merge_commit_sha"]
            == evidence["interruption"]["acknowledged_sha"],
            "ACK_EFFECT_SHA_MISMATCH",
        )
        require(
            effect["merge_commit"]["sha"]
            == effect["merge_commit_sha"],
            "EFFECT_DIRECT_COMMIT_SHA_MISMATCH",
        )
        require(
            effect["merge_commit"]["parents"]
            == [
                report["target"]["base_sha"],
                report["target"]["head_sha"],
            ],
            "EFFECT_DIRECT_COMMIT_PARENTS_MISMATCH",
        )
        require(
            closure["status"] == "EFFECT_CLOSED"
            and closure["active_frontiers"] == 0,
            "CLOSURE_INVALID",
        )
        require(
            record_hash(effect)
            in closure["confirmed_effect_hashes"],
            "CLOSURE_EFFECT_BINDING_INVALID",
        )
        require(
            closure["unattributed_merge_observations"] == [],
            "UNATTRIBUTED_OBSERVATION_PRESENT",
        )
        require(
            evidence["recovery"]["provider_direct_commit"]["sha"]
            == effect["merge_commit_sha"],
            "DIRECT_COMMIT_SHA_MISMATCH",
        )
        require(
            evidence["recovery"]["provider_direct_commit"]["parents"]
            == [
                report["target"]["base_sha"],
                report["target"]["head_sha"],
            ],
            "DIRECT_COMMIT_PARENTS_MISMATCH",
        )
        if require_live:
            require(
                evidence.get("independent_provider_matches") is True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    require(
        args.output is not None and args.state is not None,
        "OUTPUT_AND_STATE_REQUIRED",
    )
    require(
    ...
    require(
        ...
        and RUN_ID.fullmatch(args.run_id) is not None,
        "RUN_ID_INVALID",
    )
    ...
    require(
        not output.exists() and not state.exists(),
    ...
    api = provider_live.GitHubAPI(
        os.environ.get("OPENLINE_GITHUB_TOKEN", "")
    )
    summary: dict[str, Any] = {
        "schema": "openline.github-ack-closure-live-003.launch.v1",
        "experiment_id": EXPERIMENT,
    ...
    try:
        ...
        add_independent_provider_observation(
            api, plan, evidence, output
        )
        ...
        summary["bootstrap_mutations"] = len(
            getattr(api, "mutations", [])
        )
        write_json(
            output / "bootstrap-http.json",
            getattr(api, "observations", []),
        )
