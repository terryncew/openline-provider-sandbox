...
001B lesson enforced structurally: every provider call must succeed and be
confirmed; any failure raises ProviderCallFailed. A failed provider call can
never return success, and effect_committed is true only when the provider
...
def get_ref(repo: str, branch: str) -> str:
    ...
    try:
        ...
        raise ProviderCallFailed(f"ref lookup malformed: {data!r}"[:300]) from exc
...
def advance_branch(
    ...
    file_content: str,
    commit_message: str,
    author_name: str = "interop-001-receiver",
    author_email: str = "interop-001-receiver@localhost",
) -> dict[str, str]:
