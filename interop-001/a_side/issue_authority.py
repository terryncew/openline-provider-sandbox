"""A-side: issue genuine signed v0.4 decision receipts through the real
openline-receipt-gate implementation (olp_gate.gateway.evaluate_request),
adapting the repo's own issuance test harness.

Produces, per case:
  <case>-receipt.json  signed proof_to_policy_decision_receipt
  <case>-action.json   the presented action bound to the authorization
SETUP.json is written once: pinned issuer key, pinned policy hash, covered effect.
"""
...
COVERED_TARGET = "terryncew/openline-provider-sandbox:refs/heads/interop-001-effect"
...
def shared_context(root: Path) -> dict:
    """One artifact, one policy, one run for all cases. Cases differ only by
    action id, session, one-use code, and the DENY flag."""
    ...
    run_id = "run-interop-001"
    settings = {"effect": "branch advance", "ref": "refs/heads/interop-001-effect"}
    capsule_hash = sha256_hex(b"capsule:interop-001")
    policy = PolicySpec.from_mapping(
        {
            "policy_id": "interop-001.receiver-policy",
            "version": "1",
...
def issue(case: str, decision_kind: str, root: Path, gate_key: Ed25519PrivateKey,
    ...
    outcome = issue_outcome_receipt(
        ...
        witness_id="interop-001-receiver",
    ...
    code = sha256_hex(f"one-use-{case}-{now.isoformat()}".encode())[:64]
    ...
    receipt = evaluate_request(
        ...
        issuer_id="interop-001-test-gate",
...
def main() -> int:
    ...
    source_method = "did:example:interop-001-source#key-1"
    ...
    if not setup_path.exists():
        setup = {
            "pinned_issuer_key": public_key_hex(gate_key),
            "pinned_policy_hash": presented["policy_hash"],
            "covered_effect": {
                "tool": COVERED_TOOL,
                "target": COVERED_TARGET,
                "repo": "terryncew/openline-provider-sandbox",
                "branch": "interop-001-effect",
            },
            "a_implementation": "openline-receipt-gate @ d618ce835424c8f9b7c0fa77133d69db379c7697",
        }
